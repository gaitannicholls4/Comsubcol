...
if ((time() >= strtotime(get_post_meta($wdm_auction->ID, 'wdm_listing_ends', true))) || in_array('expired', $active_terms)) {
    ...
    $auc_time = 'exp';
    ?>
    <div class="wdm-auction-ending-time"><?php printf(__('Ended at', 'wdm-ultimate-auction').': '.__('%s ago', 'wdm-ultimate-auction'), '<span class="wdm-single-auction-ending">'.$rem_tm.'</span>');
        ?> </div>
        ...
        <?php if (!empty($to_bid)) {
         ...
         <?php   }else{
                    ?>
                  <strong><?php echo $currency_symbol.number_format($curr_price, 2, '.', ',').' '.$currency_code_display;
                        ?></strong>
            <?php } ?>
        ...
        elseif (get_post_status($wdm_auction->ID) != 'publish') {
            ...
            <div id="wdm_place_bid_section">
             <div class="wdm_reserved_note wdm-mark-red" style="float:left;">
                <em><?php printf(__('Auction status is %s. You can not bid on this.', 'wdm-ultimate-auction'), get_post_status($wdm_auction->ID));
                    ?></em>
                </div>
            </div>
            <br />
        ...
        elseif (in_array('scheduled', $active_terms)) {
            ...
            <div id="wdm_place_bid_section">
             <div class="wdm_reserved_note wdm-mark-red" style="float:left;">
                <em><?php printf(__('Auction has not been started yet. It will start on %s.', 'wdm-ultimate-auction'), get_post_meta($wdm_auction->ID, 'wdm_creation_time', true));
                    ?></em>
                </div>
            </div>
            <br />
         ...
         <?php   }else{
                    ...
                    if( isset($is_reserve_set) && $is_reserve_set=='1'){
                        if ($curr_price >= get_post_meta($wdm_auction->ID, 'wdm_lowest_bid', true)) {
                            ...
                            <div class="wdm_reserved_note wdm-mark-green" style="float:left;">
                              <em><?php _e('Reserve price has been met.', 'wdm-ultimate-auction');
                                ?></em>
                            </div>
                            <?php
                            ...
                            ?>
                            <div class="wdm_reserved_note wdm-mark-red" style="float:left;">
                              <em><?php _e('Reserve price has not been met by any bid.', 'wdm-ultimate-auction');
                                ?></em>
                            </div>
                            <?php
                     ...
                     if ($curr_user->ID != $wdm_auction->post_author) {
                        ...
                        <form action="<?php echo dirname(__FILE__); ?>" style="margin-top:20px;">
                         <div class="wdm_bid_val" style="">
                            <label for="wdm-bidder-bidval"><?php _e('Bid Value', 'wdm-ultimate-auction');
                                ?>: </label>
                               ...
                               <?php if($reverse_enable=='no'){
                                                    if( !empty($bid_exists) && !empty($is_inc_price_enable )){ ?>
                                                                     <small class="wdm_inc_price" data-wdm-inc-price="<?php echo $inc_price; ?>" >(<?php printf(__('Enter %.2f or more', 'wdm-ultimate-auction'), $inc_price);
                                                                                    ?>)</small>
                                                     ...
                                                     <?php  }else{
                                                        ...
                                                        <small class="wdm_inc_price" data-wdm-inc-price="<?php echo $inc_price; ?>" >(<?php printf(__('Enter less than %.2f', 'wdm-ultimate-auction'), $inc_price);
                                                                        ?>)</small>
                                  <?php } ?>
                                    <br />
                                    <small>
                          ...
                          <div class="wdm_place_bid" style="float:right;">
                            <input type="submit" value="<?php _e('Place Bid', 'wdm-ultimate-auction');
                ...
                <div class="wdm_place_bid" style="float:right;padding-top:6px;">
                  <a class="wdm-login-to-place-bid login_popup_boxer" href="#ua_login_popup" title="<?php _e('Login', 'wdm-ultimate-auction');
                  ?>"><?php _e('Place Bid', 'wdm-ultimate-auction');
                  ?></a>
              </div>
              ...
              <?php if ($curr_user->ID != $wdm_auction->post_author) {
                ...
                <div id="wdm-buy-line-above" >
                    ...
                    if ($adaptive_payment) {
                        ...
                        <input type="submit" value="<?php echo $bn_text;
...
<?php
                                    ...
                                    <div id="wdm_buy_now_section">
                                     <?php if ($curr_user->ID != $wdm_auction->post_author) {
                                        ?>
                                        <div id="wdm-buy-line-above" >
                                            <form action="<?php echo add_query_arg(array('mt' => 'bn', 'wdm' => $a_key), get_permalink($wdm_auction->ID));
                                                ?>" method="post">
                                                <input type="submit" value="<?php echo $bn_text;
                                                ?>" id="wdm-buy-now-button">
                                            </form>
        ...
        if (is_user_logged_in()) {
            ...
            <div class="wdm-clear">
                ...
                <div><span class="wdm-single-auction-tax"><?php printf(__('Tax Rate (exclusive)', 'wdm-ultimate-auction').': '.__('%s', 'wdm-ultimate-auction'),$tax_amt);
                    ?>%</span></div>
                    <?php  }
                    ...
                    if ($chk_watchlist === 'n') {
                        ...
                        <div class="wdm_add_to_watch_div_lin" id="wdm_add_to_watch_div_lin">
                          <a href="#" id="wdm_add_to_watch_lin" name="wdm_add_to_watch_lin"><?php _e('Add to Watchlist', 'wdm-ultimate-auction');
                            ?></a>
                        </div>
                        <div class="wdm_rmv_frm_watch_div_lin" id="wdm_rmv_frm_watch_div_lin" style="display:none;">
                          <a href="#" id="wdm_rmv_frm_watch_lin" name="wdm_rmv_frm_watch_lin"><?php _e('Remove from Watchlist', 'wdm-ultimate-auction');
                            ?></a>
                        </div>
                        <?php
    ...
    <div class="wdm-clear">
        ...
        <div><span class="wdm-single-auction-tax"><?php printf(__('Tax Rate (exclusive)', 'wdm-ultimate-auction').': '.__('%s', 'wdm-ultimate-auction'),$tax_amt);
            ...
            <div class="wdm_add_to_watch_div_lout">
             <a href="#ua_login_popup_w" class="login_popup_boxer" id="wdm_add_to_watch_lout"  name="wdm_add_to_watch_lout" ><?php _e('Add to Watchlist', 'wdm-ultimate-auction');
                ?></a>
            ...
            <div class="wdm-single-auction-description"> -->
                <?php if (is_user_logged_in()) {
                    ...
                    if (!empty($wdm_attach_file)) {
                        ...
                        ?>
                        <a href="<?php echo $wdm_attach_file;
                        ?>" download="<?php echo basename($wdm_attach_file);
                        ?>" target="_blank"><img src="<?php echo plugins_url('img/pdf.jpg', __FILE__);
                        ?>" style="height:52px;"/></a>
                        <?php
